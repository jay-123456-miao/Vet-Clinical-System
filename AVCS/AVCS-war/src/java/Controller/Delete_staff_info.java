...
public class Delete_staff_info extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
           Long staffIdDelete = 0l;
           managingStaff staffdelete = (managingStaff)s.getAttribute("staffobj");
           managingStaffList.remove(staffdelete);
           s.setAttribute("staffList",  managingStaffList);
            if(Long.toString(staffdelete.getId())!= null)
            {
                staffIdDelete = staffdelete.getId();
                managingStaffFacade.deleteById(staffdelete.getId());
            }
